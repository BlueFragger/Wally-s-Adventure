...
public class LevelSelection : MonoBehaviour
{
    ...
    private void UpdateLevelStatus()
    {
        ...
        if (PlayerPrefs.GetInt("Lv" + previousLevelNum.ToString()) > 2)//If the firts level star is bigger than 0, second level can play
        {
            unlocked = true;
        }
    }
    ...
    public void PressSelection(string _LevelName)//When we press this level, we can move to the corresponding Scene to play
    {
        if(unlocked)
        {
             MMSceneLoadingManager.LoadScene(_LevelName);
        }
    }
